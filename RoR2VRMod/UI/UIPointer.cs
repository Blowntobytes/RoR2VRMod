...
namespace VRMod
{
    internal class UIPointer
    {
        ...
        {
            // Laser-pointer point-and-click is DISABLED. Menus are driven gamepad-style (left stick /
            // A / B) instead - see ControllerGlyphs.ChangedToCustom. The pointer used to overwrite the
            // cursor position every frame from the hand's aim ray; with the hands mispositioned the
            // cursor landed nowhere, which made every menu un-clickable and even overrode the stick
            // navigation. Leaving this hook off removes that failure mode entirely.
            //
