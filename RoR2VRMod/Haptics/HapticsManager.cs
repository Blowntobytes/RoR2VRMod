    ...
    class HapticsManager
    {
        ...
        internal static void Init()
        {
            if (ModConfig.HapticsSuit.Value == "Shockwave")
                hapticsController = CreateShockwave();
            else if (ModConfig.HapticsSuit.Value == "Bhaptics")
                hapticsController = CreateBHaptics();
            else
                return;
        ...
        // Isolated factory methods — each lives in its own method body so the JIT
        // only resolves the haptics-suit type when the method is actually invoked.
        // This prevents TypeLoadException from killing Init() when the backing DLL
        // (ShockwaveManager.dll or Bhaptics.Tact.dll) is absent.
        private static GenericHapticsController CreateShockwave() => new ShockwaveController();
        private static GenericHapticsController CreateBHaptics() => new BHapticsController();
