...
namespace VRMod
{
    internal class MotionControlledAbilities
    {
        ...
        // ProjectileGrappleController.BaseState is a private nested type; depending on the HookGen run,
        // MMHOOK types `self` as the nested type itself or as the nearest public base
        // (EntityStates.BaseState). Taking the base type binds against either via contravariance, and
        // `orig` is called with the downcast value so both delegate shapes compile.
        private static Ray GetGrappleArmRay(On.RoR2.Projectile.ProjectileGrappleController.BaseState.orig_GetOwnerAimRay orig, EntityStates.BaseState self)
        {
            ...
            CharacterBody body = state != null ? state.owner.characterBody : null;
